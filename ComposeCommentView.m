...
//  Blocstagram
//
//  Created by Amanda Pi on 2015-03-05.
...
- (NSAttributedString *) commentAttributedString {
    ...
    [commentString addAttribute:NSKernAttributeName value:@1.3 range:range]; // character spacing
...
- (void) layoutSubviews {
    ...
    // the CGRect is larger than comment button. So text will wrap.
...
- (void) stopComposingComment { // if editing stops, dismiss keyboard
...
- (void) setIsWritingComment:(BOOL)isWritingComment {
    // setIsWritingComment calls setIsWritingComment:animated
...
- (void) setIsWritingComment:(BOOL)isWritingComment animated:(BOOL)animated {
    _isWritingComment = isWritingComment;  // store new setting
    
    if (animated) {
        [UIView animateWithDuration:0.2 animations:^{
            [self layoutSubviews]; // call layoutSubviews to update view positions
        }];
    } else {
        [self layoutSubviews];
    }
...
- (void) setText:(NSString *)text {
    _text = text;  // update textView
    self.textView.text = text;
    self.textView.userInteractionEnabled = YES;  // will set to NO when uploading comment to API
...
- (void) commentButtonPressed:(UIButton *) sender {
    if (self.isWritingComment) { // user done writing
        [self.textView resignFirstResponder];  // dismiss keyboard
        self.textView.userInteractionEnabled = NO;  // send comment to API
        [self.delegate commentViewDidPressCommentButton:self];
    } else {  // user hasn't started writing, so bring up keyboard
        [self setIsWritingComment:YES animated:YES];
        [self.textView becomeFirstResponder];
    }
}

#pragma mark - UITextViewDelegate Protocol: inform delegates of user actions

- (BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    [self setIsWritingComment:YES animated:YES];
    [self.delegate commentViewWillStartEditing:self];
    return YES; // to allow user to do what they want
}
