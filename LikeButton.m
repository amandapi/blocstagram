...
- (instancetype) init {
    ...
    if (self) {
        ...
        self.contentEdgeInsets = UIEdgeInsetsMake(10, 10, 10, 10); // buffer between button and content
        self.contentVerticalAlignment = UIControlContentVerticalAlignmentTop; // by default it is centered, now we want it at top
        
        self.likeButtonState = LikeStateNotLiked;  // set default to notLiked
    }
    
    return self;
}

- (void) layoutSubviews { // update spinner view's frame when button's frame change
    [super layoutSubviews];
    self.spinnerView.frame = self.imageView.frame;
}

- (void) setLikeButtonState:(LikeState)likeState {
    // update button based on the set state
